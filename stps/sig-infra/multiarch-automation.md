...
| **OCP & OpenShift Virtualization Version(s)** | OCP 4.22, CNV 4.22                   | Versions supporting enableMultiArchBootImageImport                           |
...
| **Network**                                   | Default networking                   | OVN-Kubernetes (OpenShift clusters deploy only with OVN-Kubernetes)           |
| **Required Operators**                        | SSP, CDI, HCO                        | SSP operator v4.22+, CDI operator v1.60+, HCO v4.22+ (versions supporting enableMultiArchBootImageImport) |
| **Platform**                                  | AWS                                  | Platform must support multi-arch node provisioning                           |
| **Special Configurations**                    | N/A                                  | Standard configuration                                                       |

**Version note:** The feature is present in 4.22. Testing is performed on 4.22.
...
- **Multi-arch cluster availability**: Multi-arch cluster is available only for 12-hour windows, limiting the scope and duration of test runs on heterogeneous environments
...
| Requirement ID | Test Case ID | Test Category | Test Summary | Test Scenario(s) | Tier | Priority | Known Issues |
| :------------- | :----------- | :------------ | :----------- | :--------------- | :--- | :------- | :----------- |
| [CNV-75960](https://issues.redhat.com/browse/CNV-75960) | TC-01 | DataSource Lifecycle | Golden images on multi-arch cluster | Deploy multi-arch cluster with enableMultiArchBootImageImport=true. Verify all bootable volumes have architecture-specific labels (example: rhel9, rhel9-amd64, rhel9-arm64) | Tier 1 | P0 | |
|                | TC-02        | DataSource Lifecycle | Images added post-deployment | Add new DataImportCron (Windows/Alpine) on existing multi-arch cluster. Verify architecture-specific images created for available node types | Tier 2 | P1 | |
|                | TC-04        | DataSource Lifecycle | Behavior on homogeneous cluster | Enable enableMultiArchBootImageImport on single-arch (amd64) cluster. Verify only amd64 images imported, no duplicates | Tier 1 | P1 | |
|                | TC-06        | DataSource Lifecycle | Duplicate volume prevention | Enable enableMultiArchBootImageImport on cluster with existing bootable volumes. Verify no duplicate volumes created, proper architecture labels applied | Tier 1 | P0 | [CNV-75084](https://issues.redhat.com/browse/CNV-75084) |
|                | TC-10        | DataSource Lifecycle | VolumeSnapshot label validation | Verify all VolumeSnapshots have correct architecture labels matching source image architecture | Tier 2 | P1 | |
|                | TC-13        | DataSource Lifecycle | DataImportCron scheduling | Configure scheduled DataImportCron on multi-arch cluster. Verify imports execute for all architectures without conflicts | Tier 2 | P2 | |
|                | TC-03        | VM Creation | Correct image selection based on architecture | Create VMs on multi-arch cluster without specifying architecture. Verify VM automatically uses correct architecture-specific DataSource | Tier 1 | P0 | |
|                | TC-05        | VM Creation | Windows ARM incompatibility | Attempt to create Windows VM on ARM node when only x86 image available. Verify graceful failure with clear error message | Tier 2 | P1 | |
|                | TC-11        | VM Creation | Mixed architecture VM creation | Create both amd64 and arm64 VMs from same base image (centos-stream9). Verify each uses correct architecture-specific DataSource | Tier 1 | P0 | |
|                | TC-07        | Cleanup & Node Lifecycle | DataSource cleanup on node removal | Remove all arm64 nodes from multi-arch cluster. Verify arm64 DataSources cleaned up, no orphaned resources | Tier 1 | P0 | [CNV-68996](https://issues.redhat.com/browse/CNV-68996) |
|                | TC-08        | Cleanup & Node Lifecycle | DataSource persistence with partial node removal | Remove single arm64 node from cluster with multiple arm64 nodes. Verify all DataSources remain, VM creation still works. **TBD**: Node removal is non-trivial, feasibility in openshift-virtualization-tests to be confirmed | Tier 3/Destructive | P1 | |
|                | TC-12        | Cleanup & Node Lifecycle | Resource cleanup on feature disable | Disable enableMultiArchBootImageImport on multi-arch cluster. Verify proper cleanup of architecture-specific resources | Tier 2 | P2 | |
|                | TC-09        | Feature Gate Toggle | Toggle enableMultiArchBootImageImport | Enable/disable/re-enable feature multiple times. Verify idempotent behavior, no resource leaks | Tier 3 | P3 | |
...
- [ ] Tier 2 tests defined and automated (TC-02, TC-05, TC-10, TC-12, TC-13)
- [ ] Tier 3 tests defined and automated (TC-08 - TBD pending feasibility review, TC-09)
